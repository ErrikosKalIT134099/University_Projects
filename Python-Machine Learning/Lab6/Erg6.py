...
def nbpredict(xtest,model):
  ...
  for p in range(0,xtest.shape[0]):
    ...
    for i in range(0,xtest.shape[1]):
      l=l*(norm.pdf(xtest[p,i],loc=model["mu"][1,i],scale=model["sigma"][1,i])/norm.pdf(xtest[p,i],loc=model["mu"][0,i],scale=model["sigma"][0,i]))
      print "222222LLLLLLLLL",l
    if l<1:
        predictions.append(0)
    else:
        predictions.append(1)
  print "asdasdasdas",predictions
  return predictions
...
while ans == 4:
    ...
    predictions=nbpredict(xtest,dic)
    ...
    plt.show()
    print evaluate(ttest, predictions,'accuracy')
    """
    ...
    for i in range(1, 10):
     ...
     print "FOLD: ",i
     k=i;
     dic=nbtrain(X_train,T_train)
     predictions=nbpredict(X_test,dic)
     p=1
     while(p<7):
         if(p==1):
             criterion="accuracy"
         ...
         global a
         a=evaluate(T_test,predictions,criterion)
